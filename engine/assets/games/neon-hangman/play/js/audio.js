/* Neon Hangman — procedural audio (WebAudio synthesis only, zero assets).
 * IIFE module exposing NPHG_Sound: key tap / letter hit pop / miss buzz /
 * win arpeggio / lose slide + persisted mute (np_hg_mute). AudioContext
 * resumes on first user gesture (autoplay policy). */
'use strict';
var NPHG_Sound = (function () {
  var ctx = null, master = null, sfx = null, muted = false;
  try { muted = localStorage.getItem("np_hg_mute") === "1"; } catch (e) {}
  function ensure() {
    if (ctx) return true;
  ...
  return {
    ...
    hit: function (i) { tone(440 + (i % 6) * 70, 660 + (i % 6) * 70, 0.09, "sine", 0.16); },
    miss: function () { tone(190, 120, 0.16, "sawtooth", 0.13); noise(0.08, 0.05); },
    dup: function () { tone(300, 220, 0.06, "triangle", 0.1); },
    win: function () {
    ...
    toggle: function () {
      muted = !muted;
      try { localStorage.setItem("np_hg_mute", muted ? "1" : "0"); } catch (e) {}
