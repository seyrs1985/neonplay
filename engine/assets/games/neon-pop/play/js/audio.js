/* Neon Pop — procedural WebAudio engine (no audio files, all synthesized) */
'use strict';
...
const Sound = (() => {
  ...
  function setMuted(m) {
    muted = m;
    try { localStorage.setItem('np_pop_mute', m ? '1' : '0'); } catch (e) {}
  ...
  /* ---------- game SFX ---------- */
  const sfx = {
    // bigger group = deeper boom + brighter chime; chain lifts the pitch
    pop(n, chain) {
      const c = Math.min(chain || 1, 5);
      noise(0.16 + Math.min(n * 0.012, 0.16), 0.32 + Math.min(n * 0.02, 0.3), 800 + n * 45);
      tone(320 + c * 55, 940 + c * 90 + n * 6, 0.12 + Math.min(n * 0.008, 0.1), 'triangle', 0.3);
      if (n >= 8) { tone(150, 42, 0.42, 'sine', 0.5); noise(0.3, 0.35, 420); }
      if (n >= 13) tone(96, 34, 0.6, 'sine', 0.55, 0.03);
    },
    bad()     { tone(150, 88, 0.12, 'sawtooth', 0.18); },
    chainUp(k){ tone(660 * Math.pow(2, k / 12), 880 * Math.pow(2, k / 12), 0.07, 'square', 0.13); },
    perfect() { [523, 659, 784, 1046, 1318, 1568].forEach((f, i) => tone(f, f * 1.005, 0.22, 'triangle', 0.26, i * 0.09)); noise(0.5, 0.25, 1600, 0.4); },
  ...
  const ARP = [164.81, 196.0, 220.0, 261.63, 293.66, 329.63]; // E minor pentatonic-ish
  const STEP_MS = 300;
  ...
  function startPad() {
    ...
    [[82.41, 0.05], [123.47, 0.038], [164.81, 0.03]].forEach(([f, v]) => {
      const o = ctx.createOscillator();
      const g = ctx.createGain();
  ...
  function initFromStorage() {
    try { muted = localStorage.getItem('np_pop_mute') === '1'; } catch (e) {}
  }
