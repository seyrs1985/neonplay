/* Neon Pop — bootstrap: canvas scaling, input, main loop (no game logic) */
'use strict';
...
(() => {
  ...
  window.addEventListener('keydown', e => {
    if (e.code === 'Space' || e.code === 'Enter') {
      ...
      if (!e.repeat) {
        Sound.resume();
        if (G.state === 'TITLE') { startRun(); Sound.sfx.start(); Sound.startMusic(); }
        else if (G.state === 'OVER' && G.overT > 0.6) { startRun(); Sound.sfx.start(); }
